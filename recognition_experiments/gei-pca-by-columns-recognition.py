...
data_matrix = np.vstack([item[1] for item in column_data])
...
for i in range(0, sample_count):
    identifier = column_data[i][0]
    orig_data = column_data[i][1]
    column_data[i] = (identifier, orig_data, scaled_pca_data_matrix[i])
...
for i in range(0, sample_count):
    vector_1 = column_data[i][2]
    for j in range(0, sample_count):
        vector_2 = column_data[j][2]
...
for i in range(0, sample_count):
    print('processing no.{0} assigned to subject {1}'.format(i, column_data[i][0]))
    best_match_id = 0
    best_match_dist = 1.0
    for j in range(0, sample_count):
        if i == j:
        ...
        if 1.0 - distance_matrix[i, j] < best_match_dist:
            ...
            best_match_id = j
    print(str(i) + ' -> '+str(j))
    print(column_data[i][0] + ' -> ' + column_data[best_match_id][0])
    if column_data[i][0] == column_data[best_match_id][0]:
        success_count += 1
